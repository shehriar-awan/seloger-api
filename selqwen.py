...
class SelogerAPI:
    def __init__(self, concurrency, annonce_details, tasks_file, max_pages):
        # Validate inputs
        if concurrency <= 0:
        ...
        if not self.api_key:
            raise ValueError("LOBSTR_API_KEY environment variable not set!")
    ...
    def get_mime_type(file_path):
        ...
        if ext == '.csv':
            ...
            raise ValueError("Invalid file extension. Valid values are: csv or tsv.")
    ...
    def create_squid(self):
        ...
        if not resp.ok:
            raise Exception(f"Error creating squid: {resp.text}")
        
        self.squid_id = resp.json().get("id")
        if not self.squid_id:
            raise Exception("Squid ID not found!")
    ...
    def update_squid(self):
        ...
        resp = requests.post(url, headers=self.headers, json=payload)
        if not resp.ok:
            raise Exception(f"Error updating squid: {resp.text}")
    ...
    def poll_task_upload_status(self, task_upload_id):
        ...
        while elapsed < max_wait:
            ...
            if not resp.ok:
                raise Exception(f"Error checking upload status: {resp.text}")
        ...
        raise Exception("Tasks file upload did not complete within expected time.")
    ...
    def delete_squid(self):
        ...
        if not resp.ok:
            raise Exception(f"Error deleting squid: {resp.text}")
        
        print(f"Squid deleted: {resp.json()}")
    ...
    def start_run(self):
        ...
        resp = requests.post(url, headers=self.headers, json=payload)
        if not resp.ok:
            raise Exception(f"Error starting run: {resp.text}")
    ...
    def poll_run_progress(self):
        ...
        while True:
            ...
            if not resp.ok:
                raise Exception(f"Error retrieving run stats: {resp.text}")
    ...
    def poll_export_status(self):
        ...
        while elapsed < max_wait:
            resp = requests.get(url, headers=self.headers)
            if not resp.ok:
                raise Exception(f"Error retrieving run details: {resp.text}")
            ...
            if data.get("export_done", False):
                ...
                print(f"Credit Used: {data.get('credit_used')}")
        ...
        raise Exception("Export did not complete within expected time.")
    ...
    def get_s3_url(self):
        ...
        resp = requests.get(url, headers=self.headers)
        if not resp.ok:
            raise Exception(f"Error requesting download URL: {resp.text}")
        
        s3_url = resp.json().get("s3")
        if not s3_url:
            raise Exception("S3 URL not found!")
    ...
    def download_csv(self, s3_url):
        ...
        if not resp.ok:
            raise Exception(f"Error downloading CSV file: {resp.text}")
...
def main():
    ...
    try:
        ...
        if api.tasks_file:
            ...
            api.delete_squid()
            print("No tasks file provided. Exiting.")
            return
